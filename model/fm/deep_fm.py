...
class DeepFM(torch.nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        linear_output = self.linear(wide_sparse_inputs)

        fm_inputs = combine_inputs([linear_output, deep_sparse_embedded_inputs])
        dnn_inputs = combine_inputs([deep_sparse_embedded_inputs, dense_inputs])

        x = self.dnn(dnn_inputs) + self.fm(fm_inputs)
        x = torch.sigmoid(x)
        return x
